...
namespace CemSys3.Controllers
{
    public class ReduccionController : Controller
    {
        ...
        private readonly IComplementoTramite<ReduccionDTO> _complementoTramite;

        public ReduccionController(
        ...
        ITarea tareaService,
        IFirmantes firmante,
        IComplementoTramite<ReduccionDTO> complementoTramite)
        {
        ...
        {
            var vm = new ReduccionVM();
            vm.SweetAlert = TempData.GetSweetAlert();

            try
            {
                var strategy = _strategyFactory.GetCreateStrategy((int)TipoTramiteEnum.Reduccion);

                if (strategy is ITramiteCreateStrategy<ReduccionDTO> typedStrategy)
                {
    ...
    : TipoTrasladoEnum.Ninguno;
        ...
        {
            int usuarioId = HttpContext.Session.GetInt32("IdUsuario") ?? 0;
            ITramiteStrategy strategy = _strategyFactory.GetStrategy((int)TipoTramiteEnum.Reduccion);
